...
//Does this object in t1 come within 2 units of t2? If not, we don't need to collide it with small objects in t2.
bool GBObjectWorld::ObjectReachesTile(GBObject * ob, int t1, int t2) {
	if ( t2 == tilesX * tilesY ) return true;
	int dt = t2 - t1;
	if ( (dt == 1 || dt == tilesX + 1) && ob->Right() < (t2 % tilesX) * kForegroundTileSize - 2
		 || dt == tilesX - 1 && ob->Left() < (t1 % tilesX) * kForegroundTileSize + 2 )
		return false;
